...
contract ERC1155SignedSwap is EIP712 {
    ...
    // Shows readable array fields in MetaMask (uint256[] instead of bytes32 hashes)
    bytes32 private constant ORDER_TYPEHASH = keccak256(
        "Order(address maker,address tokenGive,uint256[] tokenIdGive,uint256[] amountGive,address tokenWant,uint256[] tokenIdWant,uint256[] amountWant,address taker,uint256 nonce,uint256 deadline)"
    );

    function _hashOrder(Order calldata order) internal view returns (bytes32) {
        // Arrays are hashed internally (still secure)
